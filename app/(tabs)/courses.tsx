...
export default function CoursesScreen() {
  ...
  const { user, token, coursesUpdatedAt } = useUserStore();

  // Function to fetch courses, optionally show loader
  ...
  // First load + background refresh on coursesUpdatedAt
  useEffect(() => {
    fetchCourses(initialLoading); // show loader on first load
  }, [token, coursesUpdatedAt]);
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-50">
      <Header coins={rewards?.totalCoins || 0} diamond={rewards?.totalDiamonds || 0} />
